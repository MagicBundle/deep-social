...
5. **Privacy (non-negotiable for live location)** — sharing is granular: anonymous Observer (the default — interests only, ~500 m blur) / precise Beacon / invisible Ghost; friends always see you; presence auto-expires; no location history retention.
...
8. **The Hot Layer** — a deterministic simulation engine that turns heartbeat + vibe events into live social-density "Social Events" on the map. Full architectural proposal in [docs/HOT-LAYER.md](docs/HOT-LAYER.md); evaluated, not yet scheduled.
