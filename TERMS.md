# Deep Social — Terms of Use (Beta)

*Last updated: July 2026*

Deep Social is a **hobby project in beta testing** — a map-forward app for
finding people and meetups nearby. These terms explain, in plain language, the
deal between you and the project author when you use it. By signing in you agree
to them. See also the [Privacy Notice](PRIVACY.md).

## What the service is

- It's **free** and provided **as-is**, for testing. There is no company behind
  it and no paid tier.
- It's a work in progress: features **may break, change, or disappear**, and
  **the database may be reset at any time**. Don't rely on it for anything
  important.

## Who can use it

- **You must be 16 or older.** The app asks you to confirm this before you
  sign in, and the confirmation is recorded on your profile. Deep Social is
  not directed at children, and under-16s may not use it (in Luxembourg,
  processing a younger person's data would also require parental consent —
  which this service does not support).
- You sign in with a **Google or Apple account**, so you need to be able to hold
  one (and to meet that provider's own age and eligibility rules).

## Your content and how you may use the app

- **You own the content you create** — event pins, photos ("vibe checks"),
  messages, profile details. You keep owning it.
- By posting it, you grant the service the right to **store it and display it to
  other users** as the app is designed to (for example, showing your pin on the
  map or your photo in an event card). That's the only thing this permission is
  for.
- You agree **not** to post or do any of the following:
  - anything **illegal**, or content you don't have the right to share;
  - **harassment, threats, hate**, or bullying of anyone;
  - **sexual content involving minors** — ever, in any form;
  - **spam**, scams, or bulk unsolicited messages;
  - **impersonating** someone else or misrepresenting who you are;
  - posting **other people's personal data** without their consent.
- **Location and event pins must not put anyone in danger** — don't send people
  to unsafe places, private property, or set up anything that could get someone
  hurt.

## Moderation, reporting, and your rights in that process

- You can **report anything**: event pins and photos (⚑ on the event card),
  people (Report on their card or profile), and message conversations (⚑ in
  the chat header). Pick a reason; adding detail helps. The person you report
  is **not told who reported them**.
- **Reports are reviewed and acted on within 24 hours.** Confirmed violations
  are removed and the responsible account is dealt with (warning, content
  removal, suspension, or termination, depending on severity).
- **If your content or account is actioned, you will be told what was
  actioned and why** (a short statement of reasons), through the app or the
  contact channel below.
- **If you think we got it wrong, reply through the contact channel** — say
  which decision you're contesting and why, and it will be re-reviewed by a
  human. You also always have the right to complain to the Luxembourg
  Digital Services Coordinator or to a court.
- Content may also be removed and accounts suspended at the operator's
  discretion for anything in the list above, with the same statement of
  reasons.

## Ending your use

- **You can delete your account at any time** — profile menu → **Delete
  account** — which removes your data in one cascade (see the Privacy Notice).
- We may **suspend or terminate** your access if you break these terms or put
  others at risk.

## Safety features are not a safety guarantee

**Guardian mode** (and any other safety-flavoured feature) is a **convenience
tool, not a safety service or guarantee**. It relies on your phone's battery,
signal, and permissions and on third-party networks, and its messages,
location updates, and check-in alerts **may be delayed, incomplete, or fail
entirely**. Do not rely on it in an emergency or as a substitute for
contacting the emergency services (112 in the EU) or a trusted person. The
project author does not monitor your meetups and is not responsible for your
safety; you remain responsible for your own choices about who you meet, where,
and when.

## No warranty, and limits on liability

- The service comes with **no warranty of any kind**. It might be unavailable,
  lose data, or behave in unexpected ways.
- To the extent the law allows, the project author is **not liable** for any
  loss or harm arising from your use of the app. You use it at your own risk,
  and you're responsible for how you interact with the people you meet through
  it — online and in person.

## Changes to these terms

These terms may change as the app evolves. The *Last updated* date above tells
you when. Continuing to use the app after a change means you accept it.

## Contact

Questions, reports, or requests: open an issue at
[github.com/MagicBundle/deep-social/issues](https://github.com/MagicBundle/deep-social/issues).
